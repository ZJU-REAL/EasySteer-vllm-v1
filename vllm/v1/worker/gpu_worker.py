...
class Worker(WorkerBase):
    ...
    def _model_hook_runner(self, feature: str) -> "GPUModelRunnerV2":
        from vllm.v1.worker.gpu.model_runner import GPUModelRunner as GPUModelRunnerV2

        runner = self.model_runner
        if (
            not isinstance(runner, GPUModelRunnerV2)
            or self.vllm_config.is_mm_encoder_only
        ):
            raise RuntimeError(
                f"{feature} requires the V2 model runner with model hooks. "
                "The V1 and multimodal encoder-only runners are not supported."
            )
        return runner

    def preload_steer_vectors(self, payloads: list[dict]) -> bool:
        return self._model_hook_runner("Steering").preload_steer_vectors(payloads)

    def get_steering_model_info(self) -> dict:
        manager = self._model_hook_runner("Steering").steer_vector_manager
        return manager.model_info() if manager is not None else {}

    def list_steer_vectors(self) -> set[int]:
        return self._model_hook_runner("Steering").list_steer_vectors()

    # Capture stream RPCs
    def start_capture(self, stream: str, **config_kwargs) -> bool:
        return self._model_hook_runner("Capture").start_capture(stream, **config_kwargs)

    def stop_capture(self, stream: str) -> bool:
        return self._model_hook_runner("Capture").stop_capture(stream)
    ...
    def capture_status(self, stream: str) -> dict:
        return self._model_hook_runner("Capture").capture_status(stream)
