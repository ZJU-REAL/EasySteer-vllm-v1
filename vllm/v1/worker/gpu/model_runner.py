...
class GPUModelRunner(
    ...
    def add_requests(self, scheduler_output: SchedulerOutput) -> None:
        for new_req_data in scheduler_output.scheduled_new_reqs:
            ...
            capture_supported = new_req_data.prompt_embeds is None
            capture_session.add_request(
                req_id,
                new_req_data.capture_select or {},
                capture_supported=capture_supported,
            )
            if capture_supported and new_req_data.num_computed_tokens > 0:
                assert new_req_data.prompt_token_ids is not None
    ...
    def execute_model(
        ...
        if not dummy_run and (has_steering or capture_enabled):
            ...
            if (
                ...
                # Adaptive verification's CPU counts are bounds/even budgets;
                # only the device offsets identify each request's actual rows.
                # Steering and capture share this one explicit host snapshot.
        ...
        if batch_desc.cg_mode == CUDAGraphMode.FULL:
            ...
            assert dispatch_manager is not None
