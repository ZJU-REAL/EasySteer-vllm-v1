...
class InputProcessor:
    def __init__(
        ...
        *,
        ...
        if steer_config is not None and steer_config.has_default_config:
            ...
            assert steer_config.steering_config is not None
    ...
    def process_inputs(
        ...
        if prompt_embeds is not None and (
            steer_vector_request is not None
            or self.capture_policy.streams
            or capture_select is not None
        ):
            raise VLLMValidationError(
                "Steering and capture require token-ID prompts; "
                "prompt embeddings are not supported."
            )
