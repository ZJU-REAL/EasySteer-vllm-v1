...
positions each config steered at each layer. Tracing is off unless the
env var is set; overhead (GPU syncs) only occurs when enabled.
"""
...
def _get_file():
    ...
    if _file is None:
        ...
        _file = open(  # noqa: SIM115 - Process-level file; close() owns cleanup.
...
def close() -> None:
    """Close the trace file; later records reopen it without resetting the step."""
    global _file
    with _lock:
        file, _file = _file, None
        if file is not None:
            file.close()
