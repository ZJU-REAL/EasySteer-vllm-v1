...
if TYPE_CHECKING:
    from vllm.model_hooks.steering.controllers import SteeringController
...
_CONTROLLERS: dict[str, "SteeringController"] = {}


def register_controller(key: str, controller: "SteeringController") -> None:
    _CONTROLLERS[key] = controller


def unregister_controller(key: str, controller: "SteeringController") -> None:
    if _CONTROLLERS.get(key) is controller:
...
def steer_moe_gate(logits: torch.Tensor, layer_name: str) -> None:
    ...
    if controller is None:
        return
    controller.apply_steering(logits)
